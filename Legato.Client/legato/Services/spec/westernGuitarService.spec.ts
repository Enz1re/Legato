...
describe("BassGuitarService", () => {
    ...
    let service: IGuitarService<WesternGuitar>;
    ...
    beforeEach(inject((_$q_: ng.IQService, _$httpBackend_: ng.IHttpBackendService, _$http_: ng.IHttpService, _$cacheFactory_: ng.ICacheFactoryService) => {
        ...
        service = new WesternGuitarService(_$q_, cacheService, new GuitarResource(_$http_, antiforgery));
    }));

    it("gets fake guitar list 0-10", done => {
        $httpBackend.whenGET("http://localhost/api/Western/0/10?filterJson=%7B%7D").respond({ guitars: fakeGuitars });
    ...
    it("gets fake guitar list 0-5", done => {
        $httpBackend.whenGET("http://localhost/api/Western/0/5?filterJson=%7B%7D").respond({ guitars: get(fakeGuitars, 5) });
    ...
    it("gets empty fake guitar list 0-0", done => {
        $httpBackend.whenGET("http://localhost/api/Western/0/0?filterJson=%7B%7D").respond({ guitars: get(fakeGuitars, 0) });
